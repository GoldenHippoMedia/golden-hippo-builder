...
| D7  | **Targeting (v1)**                 | Default flow + "purchased product/bundle is X" trigger, priority tiebreak. **Extensible** (typed condition list) so category / cart-value / subscription can be added later.                                                                                                                                                                         | Covers the stated example; additive by design.                                                                                                                                                                                                                        |
...
| `conditions`                 | list of `{ conditionType: select, … }` | **Extensible targeting (D7).** v1: `conditionType: 'Purchased Product'` + a `products` selector, `showIf`-gated. New types added as enum values + gated fields — additive.                                                                                                                                                                                                                                                    |
...
1. **Flows** (landing) — list flows grouped by brand with status (active / default / # offers). Flow detail:
   ...
   - **Conditions** editor — v1: "Purchased Product" picker. Structured so more condition types slot in later.
