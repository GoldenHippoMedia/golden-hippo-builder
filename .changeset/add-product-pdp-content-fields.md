...
- `supplementFacts` — an optional object containing a `supplementFactsLabel` list, where each item has a `supplementFactsLabelTitle` (text) and `supplementFactsLabelHtml` (HTML). Both inner fields are localized. Used to render supplement facts labels on the PDP.
