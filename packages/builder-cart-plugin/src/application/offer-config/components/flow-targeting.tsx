        ...
        {conditions.map((condition, ci) => (
          <div key={ci} className="rounded-xl border border-[var(--border-glass)] bg-[var(--bg-glass)] p-4">
            <div className="mb-3 flex items-center gap-2">
              ...
              <button type="button" className={iconButtonClass} disabled={disabled} onClick={() => removeCondition(ci)}>
                Remove
              </button>
            </div>

            <div className="flex flex-col gap-2">
              {(condition.products ?? []).map((entry: any, pi: number) => (
                <div key={pi} className="flex flex-wrap items-center gap-2">
                  <select
                    className="hippo-input min-w-[200px] flex-1"
                    value={productRefId(entry)}
                    disabled={disabled}
                    onChange={(e) => setProduct(entry, e.target.value)}
                  >
                    <option value="">{sortedProducts.length ? 'Select a product…' : 'No products available'}</option>
