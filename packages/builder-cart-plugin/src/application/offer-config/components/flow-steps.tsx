...
const FlowSteps: React.FC<FlowStepsProps> = observer(({ data, templates, markDirty, disabled }) => {
  ...
  const addOffer = (step: any) => {
    if (!step.offers) step.offers = [];
    step.offers.push({ offer: '' });
    markDirty();
  };
                  ...
                  {offers.map((entry, oi) => (
                    <React.Fragment key={oi}>
                      ...
                      <div className="flex items-center gap-1.5">
                        {/* Placeholder id input — the searchable offers picker (CommerceApi) lands in the next step. */}
                        <input
                          type="text"
                          className="hippo-input py-1.5! text-xs"
                          placeholder="Offer id…"
                          value={entry.offer ?? ''}
                          disabled={disabled}
                          onChange={(e) => {
                            entry.offer = e.target.value;
                            markDirty();
                          }}
                        />
                        <button
