...
const OfferPicker: React.FC<OfferPickerProps> = observer(({ step, stepLabel, offers, onToggle, onClose }) => {
    ...
    <>
      ...
      <aside className="fixed right-0 top-0 z-[60] flex h-full w-[440px] max-w-[92vw] flex-col border-l border-[var(--border-glass)] bg-[var(--bg-secondary)] shadow-2xl">
        ...
        <div className="flex-1 overflow-y-auto px-3 pb-4">
          {matches.length === 0 && (
            <div className="py-12 text-center text-sm text-[var(--text-muted)]">
              {offers.length === 0 ? 'No offers available.' : 'No offers match your search.'}
            </div>
          )}
          {matches.map((offer) => {
        ...
        <div className="flex items-center gap-3 border-t border-[var(--border-glass)] p-4">
          ...
          <button
            onClick={onClose}
            className="rounded-lg bg-[var(--accent)] px-4 py-2 text-sm font-semibold text-[#1a1300] hover:brightness-110"
          >
            Done
          </button>
        </div>
