  ...
  return useObserver(() => {
    ...
    const saveButton = store.canWrite ? (
      <button
        ...
        onClick={save}
        className="cursor-pointer rounded-lg bg-[var(--accent)] px-5 py-2 text-sm font-semibold text-[#1a1300] transition-all hover:brightness-110 disabled:cursor-not-allowed disabled:opacity-40 disabled:hover:brightness-100"
      >
    ...
    return (
      <div>
        <button
          type="button"
          onClick={onBack}
          className="mb-4 inline-flex cursor-pointer items-center gap-1.5 text-sm text-[var(--text-secondary)] transition-colors hover:text-[var(--text-primary)]"
        >
        ...
        <div className="mt-6">
          <FlowSteps
            ...
            offersLoading={offerStore.loading}
            offersError={offerStore.error}
            editUrl={editUrl}
            markDirty={markDirty}
            disabled={!store.canWrite}
          />
