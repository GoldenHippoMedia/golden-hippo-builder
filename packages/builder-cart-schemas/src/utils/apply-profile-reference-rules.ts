...
export function applyProfileReferenceRules(
  profile: Partial<PetProfile>,
  rules: BuilderProfileReferenceRuleContent[],
): AppliedRules {
  ...
  for (const rule of rules) {
    ...
    const isRuleSatisfied = applicationRules.every((applicationRule) => {
      ...
      if (
        field === ProfileFieldReference.Gender ||
        field === ProfileFieldReference.Type ||
        field === ProfileFieldReference.Breed
      ) {
        ...
        if (stringOperator === 'contains')
          return profile[field]?.toLowerCase().includes(stringValue?.toLowerCase() ?? '');
        if (stringOperator === 'does not contain')
          return !profile[field]?.toLowerCase().includes(stringValue?.toLowerCase() ?? '');
      }
      ...
      if (field === ProfileFieldReference.HealthConditions) {
        const { stringOperator, stringValue } = applicationRule;
        if (stringOperator === 'is')
          return profile[field]?.some((condition) => condition.toLowerCase() === stringValue?.toLowerCase());
        if (stringOperator === 'is not')
          return !profile[field]?.some((condition) => condition.toLowerCase() === stringValue?.toLowerCase());
        if (stringOperator === 'contains')
          return profile[field]?.some((condition) => condition.toLowerCase() === stringValue?.toLowerCase()); // Same as "is"
        if (stringOperator === 'does not contain')
          return !profile[field]?.some((condition) => condition.toLowerCase() === stringValue?.toLowerCase()); // Same as "is not"
      }
    });
